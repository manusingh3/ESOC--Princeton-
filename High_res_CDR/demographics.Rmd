...
Using a similar methodology as outlined in the economic activity section above, we compare population densities of the sampled locations in Kabul, Parwan vs the rest of the coutnry. 
...
In line with expectation the t test shows that the null hypothesis is rejected and the sampled group and national levels are not exactly comparable. There can be multiple factors explaining this difference, the primary one being a large number of very small values of far flung remote areas which are being compared to electrified and relatively progressive zones in Kabul and Parwan. Zero levels have been expluded from the analysis. 
...
```
####Age wise attrition 
To check if a significant portion of a certain age group is leaving the study we chek the density plot. The two groups are observed to very similar. 
